...
public class SetFontSizeCommand implements Command {
    ...
    public SetFontSizeCommand(AppService appService, int oldValue, int newValue, Font oldFontContext) {
    ...
    public void execute() {
        ...
        if (appliedToSelection) {
            for (Shape shape : targetShapes) {
                ...
                shape.setFont(new Font(currentShapeFont.getFamily(), currentShapeFont.getStyle(), newValue));
            }
        } else {
            baseService.getDrawing().setFont(new Font(oldFontContext.getFamily(), oldFontContext.getStyle(), newValue));
        }
        triggerRepaint();
    }
        ...
        if (appliedToSelection) {
            for (Shape shape : targetShapes) {
                ...
                if (currentShapeFont != null) {
                    shape.setFont(new Font(currentShapeFont.getFamily(), currentShapeFont.getStyle(), oldValue));
                }
            }
        } else {
